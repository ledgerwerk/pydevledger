...
def test_install_command_is_deterministic_and_uses_link_mode(tmp_path: Path, monkeypatch) -> None:
...
def test_dry_run_does_not_execute(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr("pydevledger.uv.shutil.which", lambda name: "/bin/uv")
    run = lambda *args, **kwargs: pytest.fail("subprocess should not run during dry-run")
    monkeypatch.setattr("pydevledger.uv.subprocess.run", run)

    assert UvPackageManager(UvConfig()).sync(Path("/python"), [project(tmp_path, "a")], dry_run=True) == 0
...
def test_install_failure_skips_check(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("pydevledger.uv.subprocess.run", run)
    assert UvPackageManager(UvConfig()).sync(Path("/python"), [project(tmp_path, "a")]) == 7
    assert len(calls) == 1
    assert calls[0][1:3] == ["pip", "install"]


def test_check_runs_after_successful_install_and_returns_check_status(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr("pydevledger.uv.shutil.which", lambda name: "/bin/uv")
    calls: list[list[str]] = []
    ...
    monkeypatch.setattr("pydevledger.uv.subprocess.run", run)
    assert UvPackageManager(UvConfig()).sync(Path("/python"), [project(tmp_path, "a")]) == 3
...
def test_missing_uv_fails_without_fallback(tmp_path: Path, monkeypatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="Configured package manager 'uv' was not found"):
